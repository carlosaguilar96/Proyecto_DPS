...
export default function App() {
  ...
  let verificarAdmin = async () => {
    try {
      ...
      if (response.data.usuarios.length == 1) // CAMBIAR LUEGO POR == 1
        setHayAdmin(false);
  ...
  else // Se crea el primer administrador
    return (
      <NavigationContainer>
        <Drawer.Navigator initialRouteName='FirstAdmin'>
          <Drawer.Screen name='FirstAdmin' component={PrimerAdminForm} initialParams={{ change: setHayAdmin }} />
        </Drawer.Navigator>
      </NavigationContainer>
    );
}
